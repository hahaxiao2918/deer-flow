...
2. Emit `schema_version`, `analysis_id`, `artifact_type: evolution_map`, `status`, corpus reference, one declared primary date basis, time-slice rules, `TransitionClaim` records, counterevidence, sparse periods, and limitations.
...
5. Form a `TransitionClaim` only when at least two comparable slices support a change. Include prior state, later state, supporting evidence, counterevidence, alternative explanation, and confidence.
