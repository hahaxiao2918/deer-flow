...
2. Emit `schema_version: 2.0.0`, `analysis_id`, `artifact_type: weak_signal_register`, `status`, baseline, corpus reference, signal records, counterevidence, alternative explanations, falsifiers, monitoring triggers, and limitations.
...
5. Build each signal record with exactly: `signal_id`, `state`, `mechanism`, `baseline_contrast`, `evidence_ids`, `counterevidence_ids`, `alternative_explanations`, `independence_assessment`, `falsification_condition`, `monitoring_trigger`, and `low_regret_action`.
