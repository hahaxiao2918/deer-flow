...
The five patent Skills under `skills/public` are the canonical `patent-research.v2` source. Their pipeline is `RetrievalManifest → LabelDecisionSet → RouteMap → TimelineAnalysis → WeakSignalRegister`; only the Retriever can call P002. Set `strict_skill_resolution: true` on the patent lead agent and its five subagents so a missing or disabled Skill rejects construction instead of silently degrading. While an active Skill narrows tools, the framework `tool_search` remains available for deferred-schema discovery, and promoted tools are still checked by the Skill policy at both model binding and execution. Configure `tool_result_sanitization.untrusted_tool_prefixes: ["patent-data_"]` so patent text is treated as untrusted remote content.
