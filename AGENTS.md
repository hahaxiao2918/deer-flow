...
- **Branding** — the root route and unauthenticated workspace guard redirect to `/loginsso`; the branded login is `frontend/src/app/(auth)/login/page.tsx` with assets in `frontend/public/images/branding/`. Do not restore the upstream landing page or change the 智海·观澜 identity without a user request.
