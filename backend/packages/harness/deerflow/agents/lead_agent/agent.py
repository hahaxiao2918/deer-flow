...
def build_middlewares(
    ...
    *,
    ...
    # estimate tracks LangGraph's true super-step count. Runs on the final
    # composed list so extension middlewares are counted too.
    for mw in composed_middlewares:
        if isinstance(mw, RecursionGuardMiddleware):
            mw.set_extra_hook_nodes(RecursionGuardMiddleware.count_model_hook_nodes(composed_middlewares, exclude=mw))
