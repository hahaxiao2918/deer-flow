...
- System prompt generated by `apply_prompt_template()` with skills, memory, and subagent instructions; the default identity is `a super agent`, while custom agents include their own name and append `SOUL.md`
...
23. **SkillToolPolicyMiddleware** - Enforces each skill's `allowed-tools` declaration only while the skill is actually active this turn (slash activation or in-context `ThreadState.skill_context`). Filters `request.tools` before model binding and blocks disallowed tool calls as errors. When no active skill declares `allowed-tools`, all bound tools remain visible
...
**Capability disclosure and routing**: `AgentConfig.subagents` uses `null` = every registered subagent, `[]` = no `task` tool, and an explicit list = whitelist. The lead-agent factory writes the effective list into server-owned run metadata; prompt construction filters against the same list and `task_tool` rejects any type outside it before config lookup, so client-supplied names cannot widen the profile.

**Progressive subagent Skills**: with `skills.deferred_discovery=true`, `SubagentExecutor._build_initial_state` binds `describe_skill` and injects only `<skill_index>`; with the flag off it retains eager `SKILL.md` loading. Middleware order is `SkillActivationMiddleware` → `DurableContextMiddleware` → `SkillToolPolicyMiddleware`, so Slash activation and `read_file(SKILL.md)` context are visible before `allowed-tools` is enforced. The Shanghai Electric patent profile source is `docs/examples/patent-research-agent/`; its five custom subagents are declared in `config.example.yaml` and contract-tested by `backend/tests/test_patent_research_profile.py`.
...
#### Skill `allowed-tools` semantics

The `allowed-tools` frontmatter field restricts which tools the model may call **only while that skill is actually active in the current turn**. A skill becomes active in two ways:
...
- `default_agent.skills` / `default_agent.subagents` / `default_agent.tool_groups` - Optional capability filters for the unchanged built-in `lead_agent` entry. All use three-state semantics (`null` = all, `[]` = none, explicit list = whitelist); omitting the block preserves upstream behavior.
