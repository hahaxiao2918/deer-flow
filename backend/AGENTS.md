...
28.5. **RecursionGuardMiddleware** - *(optional, if `recursion_guard.enabled`, default on)* Converts LangGraph's hard `GraphRecursionError` into a natural finish for long tool-heavy runs (a run killed at the step ceiling surfaced as *failed* even when deliverables and the final answer were already in state — patent smoke 2026-07-26). Reads the run's effective limit from `runtime.context["recursion_limit"]` (plumbed by Gateway `build_run_config` and `DeerFlowClient.stream`; absent = inactive), estimates consumed super-steps per model cycle (model node + own `after_model` node + other middleware hook nodes, auto-calibrated by the builders via `set_extra_hook_nodes(count_model_hook_nodes(chain))` after full chain assembly + tools node when the response calls tools — parallel calls share one node), injects a wrap-up reminder at `warn_ratio` (default 0.75), and strips `tool_calls` at `limit - hard_margin` (default 10) so the agent terminates with a final answer; stamps `recursion_capped` via `consume_stop_reason` and `context["stop_reason"]`, mirroring the loop/token guards. Note the measured topology fact behind the cost model: every middleware implementing `before_model`/`after_model` adds its own graph node per model cycle, so the true step count grows with chain length, and the server default `recursion_limit` was raised 100 → 250 (Gateway `_DEFAULT_RECURSION_LIMIT`, channels `DEFAULT_RUN_CONFIG`, embedded client default; clamp ceiling `max_recursion_limit` unchanged)
...
SSO supports two provider families via `auth.oidc.providers.<id>.provider_type`: standard **`oidc`** (Keycloak/Google/Azure — discovery + ID token + JWKS + nonce, see `app/gateway/auth/oidc.py`) and a custom non-OIDC **`oauth2`** type for providers like the Shanghai Electric 数字底座 (IPD) — no discovery/issuer/ID token; token exchange sends `grant_type/client_id/client_secret/code/redirect_uri/state` all in the query string with `tenant-id` in a header; the provider's `state` is not trusted (it is only echoed back to the token endpoint), so CSRF uses a DeerFlow nonce cookie (`app/gateway/auth/oauth2_state.py`) that is enforced only for DeerFlow-initiated flows (`/start`) — IPD-initiated logins (the official 用例一 portal "click app" path, which opens `/loginsso?code=...` in a fresh tab with no prior `/start` and thus no cookie) are allowed through, with the one-time code + `client_secret` as the defense; the callback is a **front-end `/loginsso` interception route** that forwards to `GET /api/v1/auth/oauth2/{provider}/{start,callback}` (`app/gateway/auth/oauth2.py`, `routers/auth.py`). The oauth2 adapter produces an `OIDCIdentity`, so user provisioning is reused unchanged and `roleCodes` are never mapped to a DeerFlow admin role. See [docs/SSO.md](docs/SSO.md) and `docs/plans/2026-07-2*-digital-foundation-sso-todo.md` / `2026-07-23-sso-ipd-implementation-plan.md`.
...
- **Remote media inputs**: HTTP/SSE MCP tools cannot see the Gateway's per-thread `/mnt/user-data` mount. Their no-session wrapper resolves only the current thread's image/video files, inlines files up to 4 MiB as `data:` URLs for the immediate MCP request, and rejects larger local media before any remote call. The original virtual path remains in agent state and persisted history; payload bytes are never put into model-visible tool arguments, checkpoints, or run records.
...
- A configured `retrieval_adapter` owns indexing and semantic retrieval. File storage sends upsert/remove notifications for normal writes and both explicit and lazy migrations after releasing durable storage locks, then delegates search; without an adapter it declares and uses `substring_fallback`. Built-in adapter: `deerflow.memory_retrieval.fastembed_retrieval.create_retrieval` (needs the `memory-retrieval` uv extra, auto-detected by deploy via `detect_uv_extras.py`) — in-process ONNX embeddings (`retrieval_model`, default `BAAI/bge-small-zh-v1.5`, 512-dim), scope-tagged linear cosine scan, process-wide singleton. The vector index is process-local; `create_storage` registers a `set_backfill` callback so the first search of a not-yet-indexed scope lazily re-embeds its durable Markdown facts after a gateway restart (single-flight per scope, idempotent). Model weights cache under `retrieval_cache_dir` (point it at a persistent dir, e.g. `.deer-flow/memory-retrieval-cache`, so containers don't re-download); compose sets `HF_ENDPOINT` (default hf-mirror.com) for the initial download.
