...
## Shanghai Electric Distribution Notes

This checkout is the Shanghai Electric distribution; the entries below are
custom deltas on top of upstream and must be preserved across upstream merges:

- **SSO / IPD 三方登录** — `auth.oidc.providers.<id>.provider_type: oauth2` is a
  custom non-OIDC provider family for the 数字底座 (IPD): no discovery/issuer/ID
  token; token exchange sends all parameters in the query string with
  `tenant-id` in a header; the provider's `state` is not trusted, so CSRF uses a
  DeerFlow nonce cookie enforced only for DeerFlow-initiated flows. The callback
...
- **RecursionGuardMiddleware** — distribution-only middleware converting hard
  ...
  Server default `recursion_limit` is 250 here (upstream: 100).
- **fastembed memory retrieval** — `deerflow/memory_retrieval/` is a
  distribution-only semantic retrieval adapter (`memory-retrieval` uv extra);
  see [packages/harness/deerflow/agents/memory/AGENTS.md](packages/harness/deerflow/agents/memory/AGENTS.md).
- **Remote MCP media inlining** — `mcp/tools.py::_make_remote_media_tool` wraps
  HTTP/SSE MCP tools so thread-local `/mnt/user-data` image/video paths are
  inlined as data URLs (≤4 MiB) before the remote call; remote servers cannot
  see the gateway's per-thread mount.
