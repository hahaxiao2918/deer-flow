...
- [ ] 底座提供的正式授权入口 URL 是什么？当前文档只清楚展示了底座携带授权码跳转第三方的过程。
- [ ] 在底座登记的精确回调 URL 是前端中转地址，还是允许直接登记 DeerFlow 后端 callback？推荐后端直接接收。
- [ ] `id`、`workId`、`username` 哪一个是不可变且唯一的外部 subject？唯一范围是全局、租户内还是组织内？
- [ ] `email` 是否对所有数字底座用户必填、合法且全局唯一？当前 DeerFlow 数据模型要求邮箱必填且唯一。
- [ ] 允许接入的 `tenant-id`、`organize-id` 白名单分别是什么？是否允许多个组织/租户？
- [ ] Token 与 UserInfo 接口的生产基础 URL、网络连通方式、TLS/证书要求和超时限制是什么？
- [ ] `Authorization` 是否严格使用标准 `Bearer <token>` 格式？
- [ ] 授权码和 state 的有效期、是否一次性、错误码全集和重试语义是什么？
- [ ] 是否要求单点登出、账号禁用即时生效或定期同步资料/角色？
- [ ] 数字底座是否提供测试客户端、测试租户、测试用户以及普通/禁用/缺邮箱/重复邮箱等验收账号？
- [ ] DeerFlow 登录页是否需要主动登录按钮；如果需要，底座必须提供授权发起入口及参数规范。
...
- 已完成：需求访谈结论、原始文档与截图取证、DeerFlow 现状调查、upstream 鉴权差异调查。
- 未开始：任何 SSO 代码、配置、迁移、页面或部署修改。
- 下一主任务：继续本地分支收敛与 upstream 更新准备。
- 解冻条件：项目负责人明确说“开工”。
